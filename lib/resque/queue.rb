...
module Resque
  ...
  class Queue
    ...
    # Get the latency of the queue in seconds
    #
    # The latency is the number of seconds that passed between the latest
    # job to be popped off the queue and the time it was originally enqueued.
    #
    # This can serve as a great metric for estimating how much time it will
    # take to process a job in this queue after enqueueing.
    def latency
      redis.get ['latency', @redis_name ].join(':').to_i
    end
